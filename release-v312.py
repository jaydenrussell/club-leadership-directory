...
if not TOKEN:
    ...
    sys.exit(1)
TAG = "v3.28.47"
NAME = f"Club Leadership Directory v3.28.47"
BODY = "Version 3.28.47: admin list plumbing no longer lets a stray form field overturn an explicit publish/unpublish action.\n\nReported: on the admin list, the status-column Unpublish icon and the toolbar Unpublish did nothing for published entries, while saving from the edit form worked. The edit-day 3.28.45 change added a hidden 'state' field next to the list form's task field. On Joomla 3 the toolbar pair Publish/Unpublish share one controller task (leadership.publish); the toolbar Unpublish button flipped only the hidden state input, so every action - grid publish, grid unpublish, toolbar publish, toolbar unpublish - landed on the same task with state always 1 and always published the row. The Unpublish buttons were effectively wired to Publish.\n\npublish() now derives the state from the task itself, which is authoritative: a task that says 'unpublish' sets published=0 regardless of the hidden field; a plain publish task with an empty state field still publishes. An explicitly posted non-empty state is only honoured when the task is a plain publish. The toolbar Unpublish now posts its own distinct task (leadership.unpublish) instead of the shared one. Verified with an 18-check harness plus php -l; the full store regression suite still passes."
