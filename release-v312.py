...
if not TOKEN:
    ...
    sys.exit(1)
TAG = "v3.28.45"
NAME = f"Club Leadership Directory v3.28.45"
BODY = "Version 3.28.45: hotfix - Unpublish from the admin list no longer crashes.\n\nThe admin list's Publish/Unpublish actions call ClubleaddirControllerLeadership::publish(), which guarded the request with $this->isPost() - a method that does not exist in Joomla 3's BaseController. Every unpublish attempt (the in-list status icon, the checkboxes plus the toolbar Unpublish button) therefore hit a 500 'Call to undefined method' and never changed the record. The guard is now the same explicit POST-method check every other action uses (plus the existing session-token check), so Unpublish actually sets published=0: the record is hidden from the front end but stays editable in the admin.\n\nThe list form also gains the hidden 'state' input (value 1) that the toolbar publish/unpublish buttons write to; when the field is absent, state safely defaults to Publish, never to anything destructive.\n\nTrash (published=-2, recoverable) remains the separate Joomla-standard Trash action; Unpublish and Delete behave as labelled.\n\nVerified: controller harness with Joomla stubs (12 checks - state=0/task-suffix unpublish, state=1 publish, absent-state default, GET rejected without crashing, unpublished record still retrievable/editable, trash distinct from unpublish), php -l clean on both touched files, and the rebuilt package contains the fix."
